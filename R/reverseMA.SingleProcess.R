reverseMA.SingleProcess <- function(n=1000,pX=0.2,gamma0=0,gammaX=0.1,varM=1,beta0=0,betaX=1,betaM=c(0,0.1,0.2),varY=1,
  ...
  for(i in 1:nSim){
    ...
    for(bM.ind in 1:length(betaM)){
      ...
      med.fitR <- (lm(M2~X))
      out.fitR <- (lm(Y2~X+M2))
      med.outR <- mediation::mediate(med.fitR,out.fitR,treat = "X",mediator = "M2",sims = nSim)
    ...
    if(floor(i/10)==ceiling(i/10)){pbapply::setTimerProgressBar(pb, value=i)}
